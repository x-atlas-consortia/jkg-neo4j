...
# Build a Zip archive for a JKG distribution.
#
...
Help()
{
   ...
   echo ""
   echo "****************************************"
   echo "HELP: JKG Zip distribution build script"
   echo "Builds a Zip distribution of the JKG neo4j Docker instance from content in the current directory.."
   echo
...
zip -3 -r "$container_name.zip" data/
zip "$container_name.zip" container.cfg.example
zip "$container_name.zip" build_container.sh

echo "The JKG distribution is available in $container_name.zip."
