...
#    This database will be a "primer" for a later import of JKG data.
...
Help()
{
   ...
   echo ""
   echo "****************************************"
   echo "HELP: JKG neo4j Docker container build script"
   echo
...
if [[ "$docker_tag" == "local" ]]
then
  docker_image_name="jkg-neo4j-local"
else
  docker_image_name="hubmap/jkg-neo4j:$docker_tag"
fi
...
if [[ "$container_name" == "" ]]
then
  echo "Error: no value for container_name. Either accept the default (jkg-neo4j) or specify a value in the config file."
  exit 1;
fi
...
then
  ...
  then
    ...
    echo "Either copy a pre-generated JKG database to '$db_mount_dir' or generate a new database."
    exit 1;
  fi
